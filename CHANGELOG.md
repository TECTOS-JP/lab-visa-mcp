...
## v2.8.2 — 取り消し後の二重通信を塞ぎ、重複ツールの複製を止める

合言葉: **「掴んだ機器は、離すまで離さない」**

### 取り消し後に同じ機器へ通信が重なり得た (P1)

VISA I/O はワーカースレッドで実行される。await 側が取り消されると
`CancelledError` がそのまま伝播し、呼び出し元の `async with self._get_lock(...)`
がロックを解放していた。**しかしスレッドは停止できないため実通信は続いており、
その隙に次のコマンドや安全停止命令が同じ機器へ二重に流れ得た。**

`_run` を修正し、取り消し時は実 I/O の完了を待ってから `CancelledError` を
送出する。ロックは実通信の終了後に解放され、同一機器への通信は常に直列化
される。修正は 1 箇所だが、ロック配下の全経路 (query / write) に効く。

取り消しの応答は実 I/O 完了まで待たされるが、機器への二重アクセスより待ち
時間の方が安全である。

### ツールモジュールの複製を止めた (P2)

`lab_visa_mcp.tools` には runtime のツール実装の写しが置かれていた。写しは
黙って乖離する — 修正は `lab_executor` に入り、写しには届かない。実際、
**bulk 取得 artifact の同梱 (P1) と監視の安全停止 (P2) は upstream にあるが、
ここに残る写しには無い。**

- upstream と完全一致していた 7 モジュール (`audit` / `dsl` / `groups` /
  `info` / `jobs` / `pdf_extractor` / `waits`) を再エクスポートに置き換えた。
  実装は `lab_executor` 側の 1 つだけになる。互換のため名前は残す。
  重複コードは 4,860 行から 2,659 行へ減った。
- 乖離している 5 モジュール (`commands` / `export` / `monitor` /
  `observation` / `recipes`) は据え置き。意図的な差分の有無を 1 つずつ確認
  する必要があり、一度に寄せると壊す危険が上回るため。
- **ドリフト検知テストを追加** (`tests/test_tool_module_drift.py`)。
  再エクスポートに実装が戻ること、未分類のモジュールが増えること、乖離が
  解消したのに写しのまま残ることを検出する。乖離 5 件は「負債」として
  テスト内に明示され、見えない状態ではなくなった。

## v2.8.1 — schemas を配布物に同梱し、検証対象ゼロを失敗にする

合言葉: **「何も調べずに通ったと言わない」**

`validate schemas` は **installed package では対象ゼロのまま成功していた**。
`schemas/` はリポジトリ直下にあり `packages = ["src/lab_visa_mcp"]` に含まれ
ないため wheel にも sdist にも入っておらず、探索先が
`Path(__file__).parent.parent.parent / "schemas"` というリポジトリ構成前提の
相対パスだったため、site-packages の隣を見て何も見つけられなかった。

`{"reports": []}` と終了コード 0 を返すので、**検証が通ったように見えて実際は
1 ファイルも見ていない**状態だった。editable install では偶然リポジトリ直下に
解決されるため、開発中は気付けない。lab-executor-mcp の CI を全テスト実行へ
切り替えた際、クリーン環境で初めて表面化した。

- `schemas/` を wheel へ force-include し、sdist の allowlist にも追加した。
- 探索はパッケージ同梱を優先し、無ければリポジトリ構成へ落とす。
- **対象が 0 件なら stderr へ理由を出して終了コード 1 を返す。** 何も検証して
  いないのに成功を返す方が、失敗するより有害であるため。

なお v2.8.0 以前から同じ挙動であり、改名によって生じた問題ではない。
