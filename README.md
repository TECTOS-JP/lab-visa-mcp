...
>
> **旧配布名 `visa-mcp` は PyPI から削除済みで、`pip install visa-mcp` は
> 失敗します。** 旧 import 名の shim も提供していません。
>
> ```bash
...
> **v2.0 で分離されました**
> v1.x まで 1 パッケージで提供していた「実験実行 runtime / DSL
> / extension ecosystem」は **`lab-executor-mcp`** に移りました。
> 本パッケージは v2.0+ で **PyVISA backend layer** に特化します。
>
> - **実機 backend が必要**: `pip install lab-visa-mcp` (自動的に
...
```bash
git clone https://github.com/TECTOS-JP/lab-visa-mcp.git
cd lab-visa-mcp
pip install -e .
```
...
```json
{
  "mcpServers": {
    "lab-visa-mcp": {
      "command": "python",
      "args": ["-m", "lab_visa_mcp.server"],
      "cwd": "<path-to-lab-visa-mcp>"
    }
  }
}
```
...
> 「lab-visa-mcp に接続されている計測器を一覧してください」
>
...
└──────────┬───────────┘
           │ MCP (stdio)
┌──────────▼───────────┐
│   FastMCP Server     │  ← src/lab_visa_mcp/server.py
│  ┌────────────────┐  │
│  │ Tool Handlers  │  │  ← src/lab_visa_mcp/tools/
│  ├────────────────┤  │
