...
`lab-executor-mcp`. Importing `lab_visa_mcp.models` now forwards to
`lab_executor.models` with a DeprecationWarning.

Migration:
    # old
    from lab_visa_mcp.models import X
    from lab_visa_mcp.models.compiler import Y
    # new
...
_warnings.warn(
    "lab_visa_mcp.models is deprecated; use lab_executor.models instead.",
    DeprecationWarning,
    stacklevel=2,
)
...
# `from lab_visa_mcp.models.<sub> import X` resolves through
...
_submodules: dict[str, object] = {
    "lab_visa_mcp.models.instrument_def": _sub_instrument_def,
}
