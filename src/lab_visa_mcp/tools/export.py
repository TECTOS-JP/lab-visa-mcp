...
from lab_visa_mcp.response_envelope import make_envelope, make_error
...
def register_tools(mcp: FastMCP, job_mgr: JobManager) -> None:
    ...
    async def get_experiment_results(
        ...
        try:
            ...
            _versions = {
                "lab_visa_mcp": getattr(_vm, "__version__", "?"),
                "lab_executor": getattr(_le, "__version__", "?"),
                "export_fix": "v2.6.0",
            }
        except Exception:
            _versions = {"lab_visa_mcp": "?", "lab_executor": "?",
                         "export_fix": "v2.6.0"}
    ...
    async def export_experiment_bundle(
        ...
        ```
        bundle.zip
        ├── manifest.json          (bundle_version / lab_visa_mcp_version / contents / checksums)
        ...
        try:
            ...
            try:
                from lab_visa_mcp.observation import build_run_summary
                steps = store.list_steps(job_id)
    ...
    async def validate_experiment_bundle(path: str) -> dict:
        ...
        try:
            ...
            if not manifest.get("lab_visa_mcp_version"):
                warnings.append({
                    "warning_class": "missing_lab_visa_mcp_version",
                    "message": "manifest に lab_visa_mcp_version が無い",
                })
        finally:
            zf.close()
        ...
        data: dict = {
            ...
            "lab_visa_mcp_version": manifest.get("lab_visa_mcp_version"),
    ...
    async def inspect_experiment_bundle(
        ...
        bundle 内容のサマリを返す:
          ...
          - manifest (bundle_version / lab_visa_mcp_version / job_id / contents)
          - plan (optional)
