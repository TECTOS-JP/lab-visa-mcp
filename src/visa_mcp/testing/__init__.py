...
`lab-executor-mcp`. Importing `visa_mcp.testing` now forwards to
`lab_executor.testing` with a DeprecationWarning.

Migration:
    # old
    from visa_mcp.testing import X
    from visa_mcp.testing.compiler import Y
    # new
...
_warnings.warn(
    "visa_mcp.testing is deprecated; use lab_executor.testing instead.",
    DeprecationWarning,
    stacklevel=2,
)
...
# `from visa_mcp.testing.<sub> import X` resolves through
...
_submodules: dict[str, object] = {
    "visa_mcp.testing.mock_instruments": _sub_mock_instruments,
    "visa_mcp.testing.benchmark_runner": _sub_benchmark_runner,
    "visa_mcp.testing.benchmark_task": _sub_benchmark_task,
}
