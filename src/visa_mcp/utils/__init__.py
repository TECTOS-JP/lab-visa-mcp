...
`lab-executor-mcp`. Importing `visa_mcp.utils` now forwards to
`lab_executor.utils` with a DeprecationWarning.

Migration:
    # old
    from visa_mcp.utils import X
    from visa_mcp.utils.compiler import Y
    # new
...
_warnings.warn(
    "visa_mcp.utils is deprecated; use lab_executor.utils instead.",
    DeprecationWarning,
    stacklevel=2,
)
...
# `from visa_mcp.utils.<sub> import X` resolves through
...
_submodules: dict[str, object] = {
    "visa_mcp.utils.condition": _sub_condition,
    "visa_mcp.utils.expression": _sub_expression,
    "visa_mcp.utils.idn_matcher": _sub_idn_matcher,
    "visa_mcp.utils.param_validator": _sub_param_validator,
}
