...
`lab-executor-mcp`. Importing `visa_mcp.dsl` now forwards to
`lab_executor.dsl` with a DeprecationWarning.

Migration:
    # old
    from visa_mcp.dsl import X
    from visa_mcp.dsl.compiler import Y
    # new
...
_warnings.warn(
    "visa_mcp.dsl is deprecated; use lab_executor.dsl instead.",
    DeprecationWarning,
    stacklevel=2,
)
...
# `from visa_mcp.dsl.<sub> import X` resolves through
...
_submodules: dict[str, object] = {
    "visa_mcp.dsl.compiler": _sub_compiler,
    "visa_mcp.dsl.schema": _sub_schema,
    "visa_mcp.dsl.template": _sub_template,
}
